...
function RPReporter(runner, options) {
    ...
    runner.on('fail', function(test, err){
      ...
      runner.on('start', function() {
        try {
          if (isShreadedTrue) {
            var readedID = fs.readFileSync('LaunchId_.txt', 'utf8');
            if (readedID.length < 1) {
              let res = connector.startLaunch();
              launchId = res.body.id;
              fs.appendFileSync('LaunchId_.txt', launchId);
            } else {
              launchId = readedID;
            }
            fs.appendFileSync('count_.txt', "+");
          }
          else
          {
            let res = connector.startLaunch();
